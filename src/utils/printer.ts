// ═══════════════════════════════════════════════════════════════
//  Pretty Printer — Print IR at each phase
//  Provides human-readable output for debugging and education.
// ═══════════════════════════════════════════════════════════════

import {
  Expr, CallExpr, ConstantExpr, VarExpr, LetExpr,
  IRModule, IRFunction, OpPattern
} from '../ir/high_level.js';
import {
  PrimFunc, ForNode, SeqNode, BufferStoreNode, AllocNode,
  BufferLoadExpr, BinOpExpr, ConstExpr as TIRConst, VarRefExpr,
  MaxExpr, MinExpr, CallExprTIR,
  VarIndex, ConstIndex, BinOpIndex,
  type Stmt, type ValueExpr, type IndexExpr
} from '../ir/low_level.js';

// ═══════════════════════════════════════
//  High-Level IR Printer
// ═══════════════════════════════════════
...
export function printHighLevelIR(module: IRModule): string {
  ...
  for (const [name, func] of module.functions) {
    ...
    const { text, varCount } = printExpr(func.body, 1);
    lines.push(text);
    lines.push('}');
    lines.push('');
  }
...
function printExpr(expr: Expr, indent: number): { text: string; varCount: number } {
  ...
  // For deeply nested call expressions, we want to print them linearly
  const assignments: string[] = [];

  function linearize(e: Expr): string {
    if (e.kind !== 'call') return visit(e);

    // First linearize arguments
...
export function printTIR(func: PrimFunc): string {
  ...
  for (const alloc of func.allocations) {
    lines.push(`  alloc ${alloc.name}: float32[${alloc.shape.join(',')}]  scope=${alloc.scope}`);
  }
...
function printStmt(stmt: Stmt, indent: number, lines: string[]): void {
  ...
  if (stmt instanceof ForNode) {
    ...
    lines.push(`${pad}for ${stmt.loopVar.name} in range(${stmt.min}, ${stmt.min + stmt.extent}):${ann}${kind}`);
    ...
    lines.push(`${pad}alloc ${stmt.buffer.name}[${stmt.buffer.shape.join(',')}] (${stmt.buffer.scope})`);
...
function printIndex(idx: IndexExpr): string {
  ...
  if (idx instanceof BinOpIndex) return `(${printIndex(idx.left)} ${idx.op} ${printIndex(idx.right)})`;
  return '?';
}
